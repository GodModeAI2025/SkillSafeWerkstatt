#!/usr/bin/env python3
"""Assess this frozen skill's bundled wiki quality without modifying it."""
...
def assess_quality(now: Optional[datetime] = None) -> dict[str, Any]:
    release = verify()
    if release.get("state") != "ready":
        return {"state": str(release.get("state") or "invalid_snapshot"), "release": release, "quality": None}
    current = (now or utc_now()).astimezone(timezone.utc)
    try:
        status = json.loads((knowledge_root() / "meta/quality-status.json").read_text(encoding="utf-8"))
        ...
        return {
            ...
            "quality": {
                ...
                "advisories": [{
                    ...
                    "severity": "warning",
                    "message": "This frozen release has no valid meta/quality-status.json.",
                    "recommended_action": "Maintain the canonical wiki and export a new knowledge skill.",
                }],
            },
        }
        ...
        return {
            ...
            "quality": {
                ...
                "advisories": [{
                    ...
                    "severity": "warning",
                    "message": "The frozen quality status uses an unsupported format.",
                    "recommended_action": "Maintain the canonical wiki and export a new knowledge skill.",
                }],
            },
        }
    ...
    identity = assess_identity(knowledge_root())
    ...
    quality_review, quality_advisories = freshness(
        "quality", reviews.get("quality") if isinstance(reviews, dict) else None,
        released_at, policy["quality_review_after_days"], current,
    )
    cleaning_review, cleaning_advisories = freshness(
        "cleaning", reviews.get("cleaning") if isinstance(reviews, dict) else None,
        released_at, policy["cleaning_review_after_days"], current,
    )
    ...
    if snapshot_age is None:
        ...
        advisories.append({
            "code": "snapshot_age_unknown", "severity": "warning",
            "message": "The frozen release timestamp is unavailable.",
            "recommended_action": "Export a new knowledge skill from a valid canonical release.",
        })
    elif snapshot_age >= policy["snapshot_warning_after_days"]:
        snapshot_state = "old"
        advisories.append({
            "code": "snapshot_old", "severity": "notice",
            "message": f"This frozen snapshot is {snapshot_age} day(s) old.",
            "recommended_action": "Check the canonical wiki for a newer release and export a new skill if needed.",
        })
    ...
    if not technical or not bool(technical.get("valid")) or safe_int(technical.get("errors")) > 0:
        advisories.append({
            "code": "technical_quality_attention_needed", "severity": "warning",
            "message": "The frozen technical quality status is missing or not valid.",
            "recommended_action": "Repair the canonical wiki, release it, and export a new knowledge skill.",
        })
    if status.get("release_id") != release.get("release_id") or status.get("version") != release.get("version"):
        advisories.append({
            "code": "quality_release_mismatch", "severity": "warning",
            "message": "The quality status does not identify this verified frozen release.",
            "recommended_action": "Release the canonical wiki and export a new knowledge skill.",
        })
