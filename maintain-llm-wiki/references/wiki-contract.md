...
- A **reserved name or character** is a file the storage layer will refuse: the names `.lock`, `CON`, `PRN`, `AUX`, `NUL`, `COM0`-`COM9`, `LPT0`-`LPT9`, any name starting with `~$`, `_vti_` anywhere in a name, the characters `" * : < > ? / \ |`, a leading or trailing space, and a trailing period. These are lint errors, because the file would silently never reach the storage. A name that is also an operating-system artifact, `desktop.ini` above all, is classified as that artifact and stays ignorable: it carries nothing, so the fact that the storage refuses it costs nothing either.
