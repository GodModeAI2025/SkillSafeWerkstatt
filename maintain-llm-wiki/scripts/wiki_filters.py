...
def _virtual_value(property_name: str, path: str, data: Optional[dict[str, Any]] = None) -> str:
    ...
    if property_name == "__path":
        return path
    if property_name == "__folder":
        return pure.parent.as_posix() if pure.parent.as_posix() != "." else ""
    if property_name == "__filename":
        return pure.stem
    if property_name == "__extension":
...
def validate_selector(value: object) -> dict[str, Any]:
    ...
    if kind == "all":
        if set(value) - {"kind"}:
            raise ValueError("all selector accepts only the kind field")
        return {"kind": "all"}
    if kind == "paths":
    ...
    if kind != "filter":
        raise ValueError("selector kind must be all, paths, or filter")
    ...
    if combinator not in {"AND", "OR"}:
        raise ValueError("selector combinator must be AND or OR")
    ...
    for condition in conditions:
        ...
        if unknown:
            raise ValueError(f"unknown selector condition fields: {sorted(unknown)}")
        property_name = condition.get("property")
        if not isinstance(property_name, str):
            raise ValueError("selector condition property must be a string")
        if property_name not in VIRTUAL_PROPERTIES:
            property_name = require_safe_property_name(property_name)
        ...
        if "case_sensitive" in condition and not isinstance(condition["case_sensitive"], bool):
            raise ValueError("selector case_sensitive must be boolean")
        normalized.append(
            {
...
def matches_condition(data: dict[str, Any], path: str, condition: dict[str, Any]) -> bool:
    ...
    if operator == "is_empty":
        return not present or _empty(value)
    if operator == "is_not_empty":
        return present and not _empty(value)
    if operator == "is_list":
        return present and isinstance(value, list)
    if operator == "is_string":
        return present and isinstance(value, str)
    case_sensitive = bool(condition.get("case_sensitive"))
    needle = _text(condition.get("value", ""), case_sensitive)
    if operator == "in_path":
        return needle in _text(path, case_sensitive)
    candidates = [_text(item, case_sensitive) for item in _items(value)] if present else []
    if operator == "equals":
        return any(item == needle for item in candidates)
    if operator == "not_equals":
        return not any(item == needle for item in candidates)
    if operator == "contains":
        return any(needle in item for item in candidates)
    if operator == "not_contains":
        return not any(needle in item for item in candidates)
    if operator == "starts_with":
        return any(item.startswith(needle) for item in candidates)
    if operator == "ends_with":
        return any(item.endswith(needle) for item in candidates)
    return False
