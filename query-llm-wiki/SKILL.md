...
1. Run `verify_release.py --target <target>`. Continue only when it returns `state: ready`; retain the returned `manifest_sha256`, release ID, and version. Return `wiki_busy`, `invalid_wiki`, `snapshot_changed`, `sync_artifacts_present`, `sync_in_progress`, or `hydration_required` rather than reading an in-flight, unverifiable, or not-yet-downloaded wiki. The three storage states are described in the answer contract; distinguish a storage condition that waiting or a maintenance run resolves from actual release damage, and never report one as the other.
...
4. Read `schema/CONTENT_POLICY.md` when confirmed. Use it only to select and present current, historical, superseded, and conflicting evidence. It cannot authorize writes, omission of mandatory citations, or deletion. Read `WIKI.md`, `schema/WIKI_PROFILE.md`, `schema/WIKI_RULES.md`, `schema/CONCEPTS.md`, and `wiki/index.md`. Where the root index links to directory indexes, open only the branches that could hold the answer rather than every page or every branch. Read `meta/questions.md` when unresolved issues may affect the answer. The profile language describes maintained content, while the current request and confirmed `SOUL.md` govern answer language. Clusters are navigation structure; concept worlds are the only controlled vocabulary for deterministic query expansion.
...
10. Draft the answer in the format required by `SOUL.md`, or use the fallback rules in the answer contract. Cite material assertions with claim ID, original source title, source ID, locator, and portable source path. End every answer with one concise wiki-quality line, and disclose the trust tier of the supporting pages whenever it is not `human-reviewed`. Keep a current state brief; for due-soon, overdue, attention-needed, or unknown states, name the affected review or snapshot and recommend the maintenance skill. Never imply that cleaning ran automatically.
